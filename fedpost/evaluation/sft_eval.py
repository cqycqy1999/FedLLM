...
class SFTGenerationEvaluator(Evaluator):
    ...
    def evaluate(self, model, round_idx: int, model_artifacts: dict | None = None):
        ...
        if self.cfg.eval.run_alpaca_eval:
            ...
            alpaca_runner = AlpacaEvalRunner(os.path.join(self.eval_dir, f"round_{round_idx}", "alpaca_eval"))
            result = alpaca_runner.run(alpaca_outputs)
        ...
        if self.cfg.eval.run_lm_eval and self.cfg.eval.lm_eval_tasks:
            ...
            lm_runner = LMEvalRunner(os.path.join(self.eval_dir, f"round_{round_idx}", "lm_eval"))
            result = lm_runner.run(
